...
public interface RegistryResourceAccess {
   ...
   static RegistryResourceAccess forResourceManager(final ResourceManager p_195882_) {
      return new RegistryResourceAccess() {
            ...
            p_195882_.listResources(s, (p_214262_) -> {
               ...
               map.put(resourcekey, (p_214266_, p_214267_) -> {
                  try {
                     ...
                     try {
                        dataresult = this.decodeElement(p_214266_, p_214267_, reader);
            ...
            return p_195882_.getResource(resourcelocation).map((p_214243_) -> {
               return (p_214247_, p_214248_) -> {
                  try {
                     ...
                     DataResult dataresult;
                     try {
                        dataresult = this.decodeElement(p_214247_, p_214248_, reader);
                     } catch (Throwable throwable1) {
                        if (reader != null) {
                           try {
                              reader.close();
         ...
         private <E> DataResult<RegistryResourceAccess.ParsedEntry<E>> decodeElement(DynamicOps<JsonElement> p_214250_, Decoder<E> p_214251_, Reader p_214252_) throws IOException {

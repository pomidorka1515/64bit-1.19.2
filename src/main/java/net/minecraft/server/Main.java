...
public class Main {
   ...
   public static void main(String[] p_129699_) {
      ...
      try {
         ...
         WorldStem worldstem;
         try {
            DataPackConfig datapackconfig = Objects.requireNonNullElse(levelstoragesource$levelstorageaccess.getDataPacks(), DataPackConfig.DEFAULT);
            WorldLoader.PackConfig worldloader$packconfig = new WorldLoader.PackConfig(packrepository, datapackconfig, flag);
            WorldLoader.InitConfig worldloader$initconfig = new WorldLoader.InitConfig(worldloader$packconfig, Commands.CommandSelection.DEDICATED, dedicatedserversettings.getProperties().functionPermissionLevel);
            worldstem = Util.blockUntilDone((p_236696_) -> {
               return WorldStem.load(worldloader$initconfig, (p_236717_, p_236718_) -> {
                  RegistryAccess.Writable registryaccess$writable = RegistryAccess.builtinCopy();
                  DynamicOps<Tag> dynamicops = RegistryOps.createAndLoad(NbtOps.INSTANCE, registryaccess$writable, p_236717_);
                  WorldData worlddata1 = levelstoragesource$levelstorageaccess.getDataTag(dynamicops, p_236718_, registryaccess$writable.allElementsLifecycle());
                  if (worlddata1 != null) {
                     ...
                     if (optionset.has(optionspec2)) {
                        ...
                        worldgensettings = WorldPresets.demoSettings(registryaccess$writable);
                     } else {
                        DedicatedServerProperties dedicatedserverproperties = dedicatedserversettings.getProperties();
                        levelsettings = new LevelSettings(dedicatedserverproperties.levelName, dedicatedserverproperties.gamemode, dedicatedserverproperties.hardcore, dedicatedserverproperties.difficulty, false, new GameRules(), p_236718_);
               ...
               }, Util.backgroundExecutor(), p_236696_);
            }).get();

...
public class Allay extends PathfinderMob implements InventoryCarrier {
   ...
   public Allay(EntityType<? extends Allay> p_218310_, Level p_218311_) {
      ...
      this.dynamicVibrationListener = new DynamicGameEventListener<>(new VibrationListener(positionsource, 16, this.vibrationListenerConfig, (VibrationListener.ReceivingEvent)null, 0.0F, 0));
   ...
   public boolean wantsToPickUp(ItemStack p_218387_) {
      ...
      return !itemstack.isEmpty() && itemstack.sameItemStackIgnoreDurability(p_218387_) && this.inventory.canAddItem(p_218387_) && this.level.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING);
   }
   ...
   public void addAdditionalSaveData(CompoundTag p_218367_) {
      ...
      VibrationListener.codec(this.vibrationListenerConfig).encodeStart(NbtOps.INSTANCE, this.dynamicVibrationListener.getListener()).resultOrPartial(LOGGER::error).ifPresent((p_218353_) -> {
   ...
   public void readAdditionalSaveData(CompoundTag p_218350_) {
      ...
      if (p_218350_.contains("listener", 10)) {
         VibrationListener.codec(this.vibrationListenerConfig).parse(new Dynamic<>(NbtOps.INSTANCE, p_218350_.getCompound("listener"))).resultOrPartial(LOGGER::error).ifPresent((p_218346_) -> {
            this.dynamicVibrationListener.updateListener(p_218346_, this.level);
         });
      }
   ...
   class AllayVibrationListenerConfig implements VibrationListener.VibrationListenerConfig {
      public boolean shouldListen(ServerLevel p_239787_, GameEventListener p_239788_, BlockPos p_239789_, GameEvent p_239790_, GameEvent.Context p_239791_) {
         if (Allay.this.getLevel() == p_239787_ && !Allay.this.isRemoved() && !Allay.this.isNoAi()) {
            Optional<GlobalPos> optional = Allay.this.getBrain().getMemory(MemoryModuleType.LIKED_NOTEBLOCK_POSITION);
            if (optional.isEmpty()) {
               return true;
            } else {
               GlobalPos globalpos = optional.get();
               return globalpos.dimension().equals(p_239787_.dimension()) && globalpos.pos().equals(p_239789_);
            }
         } else {
            return false;
         }
      ...
      public void onSignalReceive(ServerLevel p_239108_, GameEventListener p_239109_, BlockPos p_239110_, GameEvent p_239111_, @Nullable Entity p_239112_, @Nullable Entity p_239113_, float p_239114_) {
         if (p_239111_ == GameEvent.NOTE_BLOCK_PLAY) {
            AllayAi.hearNoteblock(Allay.this, new BlockPos(p_239110_));
         }
   ...
   class JukeboxListener implements GameEventListener {
      ...
      public boolean handleGameEvent(ServerLevel p_240002_, GameEvent.Message p_240003_) {
         if (p_240003_.gameEvent() == GameEvent.JUKEBOX_PLAY) {
            Allay.this.setJukeboxPlaying(new BlockPos(p_240003_.source()), true);
            return true;
         } else if (p_240003_.gameEvent() == GameEvent.JUKEBOX_STOP_PLAY) {
            Allay.this.setJukeboxPlaying(new BlockPos(p_240003_.source()), false);
            return true;
         } else {
            return false;
         }

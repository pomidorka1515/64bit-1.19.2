...
public class LevelTicks<T> implements LevelTickAccess<T> {
   ...
   private final Object2LongMap<ChunkPos> nextTickForContainer = Util.make(new Object2LongOpenHashMap<>(), (p_193262_) -> {
   ...
   public LevelTicks(Predicate<ChunkPos> p_193211_, Supplier<ProfilerFiller> p_193212_) {
   ...
   public void schedule(ScheduledTick<T> p_193252_) {
      BlockPos i = p_193252_.pos();
      LevelChunkTicks<T> levelchunkticks = this.allContainers.get(new ChunkPos(i));
      if (levelchunkticks == null) {
   ...
   private void sortContainersToTick(long p_193217_) {
      ObjectIterator<Entry<ChunkPos>> objectiterator = Object2LongMaps.fastIterator(this.nextTickForContainer);

      while(objectiterator.hasNext()) {
         Entry<ChunkPos> entry = objectiterator.next();
   ...
   public void copyArea(BoundingBox p_193243_, Vec3i p_193244_) {
      ...
      list.forEach((p_193260_) -> {
         this.schedule(new ScheduledTick<>(p_193260_.type(), p_193260_.pos().offset(p_193244_), p_193260_.triggerTick(), p_193260_.priority(), p_193260_.subTickOrder() - i + j + 1L));
      });
   }
   ...
   interface PosAndContainerConsumer<T> {
      void accept(ChunkPos k1, LevelChunkTicks<T> p_193290_);
   }
}